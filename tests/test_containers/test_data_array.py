...
class TestCoordsModel:
    ...
    def test_schema_is_valid(self, validator: Validator, model: xrm.CoordsSchema):
...
class TestDataArrayModel:
    ...
    def test_schema_is_valid(self, model: xrm.DataArraySchema):
        """Should always produce a valid JSON Schema with any combination of parameters."""
        model.check_schema()
    ...
    def test_validation_with_default_parameters_passes(self, da: xr.DataArray):
        ...
        xrm.DataArraySchema().validate(da)
...
class TestAttrs:
    ...
    def test_validation_with_allow_extra_items_passes(self, da: xr.DataArray):
        ...
        expected = xrm.AttrsSchema(allow_extra_items=True)
        xrm.DataArraySchema(attrs=expected).validate(da)
    ...
    def test_validation_with_allow_extra_items_fails(self, da: xr.DataArray):
        ...
        expected = xrm.AttrsSchema(allow_extra_items=False)
    ...
    def test_validation_with_required_keys_passes(self, da: xr.DataArray):
        ...
        expected = xrm.AttrsSchema([xrm.AttrSchema(key, required=True)])
        xrm.DataArraySchema(attrs=expected).validate(da)
    ...
    def test_validation_with_optional_keys_passes(self, da: xr.DataArray):
        ...
        expected = xrm.AttrsSchema(
            [
                xrm.AttrSchema(key, required=False),
                xrm.AttrSchema(missing_key, required=False),
            ]
        )
        xrm.DataArraySchema(attrs=expected).validate(da)
    ...
    def test_validation_with_key_regex_match_passes(
        ...
        expected = xrm.AttrsSchema([xrm.AttrSchema(key=pattern, regex=True)])
    ...
    def test_validation_with_key_regex_fails(self, da: xr.DataArray):
        ...
        expected = xrm.AttrsSchema(
            [xrm.AttrSchema(key=r'^expected$', regex=True)], allow_extra_items=False
        )
        with pt.raises(ValidationError):
            xrm.DataArraySchema(attrs=expected).validate(da)
    ...
    def test_validation_with_key_value_pairs_passes(
        ...
        expected = xrm.AttrsSchema([xrm.AttrSchema(key, value=value)])
        xrm.DataArraySchema(attrs=expected).validate(da)
    ...
    def test_validation_with_key_value_pairs_fails(
        ...
        expected = xrm.AttrsSchema([xrm.AttrSchema(key, value=expected_value)])
        with pt.raises(ValidationError):
            xrm.DataArraySchema(attrs=expected).validate(da)
    ...
    def test_validation_with_key_type_pairs_passes(
        ...
        key, value = next(iter(da.attrs.items()))
        expected = xrm.AttrsSchema([xrm.AttrSchema(key, value=type(value))])
        xrm.DataArraySchema(attrs=expected).validate(da)
    ...
    def test_validation_with_key_type_pairs_fails(
        ...
        if isinstance(value, (int, float)):
            ...
            try:
                ...
                return
        expected = xrm.AttrsSchema([xrm.AttrSchema(key, value=expected_type)])
        with pt.raises(ValidationError):
            xrm.DataArraySchema(attrs=expected).validate(da)
    ...
    def test_validation_with_duplicate_keys_passes(self, da: xr.DataArray):
        ...
        key, value = next(iter(da.attrs.items()))
        expected = xrm.AttrsSchema([xrm.AttrSchema(key), xrm.AttrSchema(key, value=value)])
        xrm.DataArraySchema(attrs=expected).validate(da)
...
class TestDataArrayChunks:
    ...
    def test_validation_with_boolean_passes(self, data: st.DataObject):
        ...
        expected = xrm.ChunksSchema(data.draw(st.booleans()))
        da = data.draw(xmst.data_arrays(dims=xrst.dimension_sizes(min_dims=1)))
        if expected.chunks:
            da = da.chunk('auto')
        xrm.DataArraySchema(chunks=expected).validate(da)
    ...
    def test_validation_with_integer_passes(self, data: st.DataObject):
        ...
        xrm.DataArraySchema(chunks=xrm.ChunksSchema(expected)).validate(da)
    ...
    def test_validation_with_sequence_of_sequence_of_integers_fails(
        ...
        with pt.raises(ValidationError):
            xrm.DataArraySchema(chunks=xrm.ChunksSchema(expected.tolist())).validate(
                da
            )
    ...
    def test_validation_with_wildcard_passes(self, data: st.DataObject):
        da = data.draw(xmst.data_arrays(dims=xrst.dimension_sizes(min_dims=1)))
        da = da.chunk(-1)
        xrm.DataArraySchema(chunks=xrm.ChunksSchema(-1)).validate(da)
...
class TestDims:
    ...
    def test_validation_with_sequence_passes(self, data: st.DataObject):
        ...
        xrm.DataArraySchema(dims=xrm.DimsSchema(expected)).validate(da)
    ...
    def test_validation_with_contains_passes(self, da: xr.DataArray):
        ...
        xrm.DataArraySchema(dims=xrm.DimsSchema(contains=contains)).validate(da)
    ...
    def test_validation_with_contains_fails(
        ...
        with pt.raises(ValidationError):
            xrm.DataArraySchema(dims=xrm.DimsSchema(contains=expected)).validate(da)
    ...
    def test_validation_with_size_constraints_passes(
        ...
        xrm.DataArraySchema(
            dims=xrm.DimsSchema(min_dims=min_dims, max_dims=max_dims)
        ).validate(da)
    ...
    def test_validation_with_size_constraints_fails(self, data: st.DataObject):
        ...
        da = data.draw(xmst.data_arrays())
        with pt.raises(ValidationError):
            xrm.DataArraySchema(dims=xrm.DimsSchema(min_dims=4)).validate(da)
...
class TestDType:
    ...
    def test_validation_passes(
        ...
        xrm.DataArraySchema(dtype=xrm.DTypeSchema(dtype_like)).validate(da)
    ...
    def test_validation_fails(self, data: st.DataObject) -> None:
        ...
        with pt.raises(ValidationError):
            xrm.DataArraySchema(dtype=xrm.DTypeSchema(expected)).validate(da)
...
class TestName:
    ...
    def test_validation_with_string_passes(self, da: xr.DataArray):
        ...
        xrm.DataArraySchema(name=xrm.NameSchema(da.name)).validate(da)
    ...
    def test_validation_with_string_fails(
        ...
        with pt.raises(ValidationError):
            xrm.DataArraySchema(name=xrm.NameSchema(expected)).validate(da)
    ...
    def test_validation_with_sequence_passes(self, data: st.DataObject):
        ...
        xrm.DataArraySchema(name=xrm.NameSchema(names)).validate(da)
    ...
    def test_validation_with_sequence_fails(self, data: st.DataObject):
        ...
        da = data.draw(xmst.data_arrays(name=st.sampled_from(names)))
        with pt.raises(ValidationError):
            xrm.DataArraySchema(name=xrm.NameSchema(name)).validate(da)
    ...
    def test_validation_with_regex_passes(
        ...
        xrm.DataArraySchema(name=xrm.NameSchema(pattern, regex=True)).validate(da)
    ...
    def test_validation_with_regex_fails(self, data: st.DataObject):
        ...
        expected = r'^expected$'
        with pt.raises(ValidationError):
            xrm.DataArraySchema(name=xrm.NameSchema(expected, regex=True)).validate(
                da
            )
    ...
    def test_validation_with_length_constraints(self, data: st.DataObject):
        ...
        xrm.DataArraySchema(
            name=xrm.NameSchema(min_length=min_length, max_length=max_length)
        ).validate(da)
...
class TestShape:
    ...
    def test_validation_with_sequence_passes(self, da: xr.DataArray):
        ...
        expected = da.shape
        xrm.DataArraySchema(shape=xrm.ShapeSchema(expected)).validate(da)
    ...
    def test_validation_with_sequence_fails(
        ...
        with pt.raises(ValidationError):
            xrm.DataArraySchema(shape=xrm.ShapeSchema(shape)).validate(da)
    ...
    def test_validation_with_item_constraints_passes(
        ...
        xrm.DataArraySchema(
            shape=xrm.ShapeSchema(max_dims=max_dims, min_dims=min_dims)
        ).validate(da)
    ...
    def test_validation_with_item_constraints_fails(self, data: st.DataObject):
        ...
        with pt.raises(ValidationError):
            xrm.DataArraySchema(
                shape=xrm.ShapeSchema(max_dims=min_dims - 1)
            ).validate(da)
            xrm.DataArraySchema(
                shape=xrm.ShapeSchema(min_dims=max_dims + 1)
            ).validate(da)
...
class TestCoords:
    ...
    def test_validation_with_allow_extra_passes(self, da: xr.DataArray):
        """Should always pass if ``AttrsSchema.allow_extra_items`` is true."""
        expected = xrm.CoordsSchema(allow_extra=True)
        xrm.DataArraySchema(coords=expected).validate(da)

    @hp.given(da=xmst.data_arrays(dims=xrst.dimension_sizes(min_dims=1)))
    def test_validation_with_allow_extra_items_fails(self, da: xr.DataArray):
        """Should fail if the instance has unspecified attributes."""
        expected = xrm.CoordsSchema(allow_extra=False)
        with pt.raises(ValidationError):
            xrm.DataArraySchema(coords=expected).validate(da)
    ...
    def test_validation_with_require_all_passes(self, da: xr.DataArray):
        ...
        expected = xrm.CoordsSchema(
            coords={name: xrm.DataArraySchema(name=xrm.NameSchema(name))},
            require_all=True,
        )
        xrm.DataArraySchema(coords=expected).validate(da)
